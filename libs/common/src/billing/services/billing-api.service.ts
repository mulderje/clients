...
export class BillingApiService implements BillingApiServiceAbstraction {
  ...
  async verifyOrganizationBankAccount(organizationId: string, request: VerifyBankAccountRequest) {
    return await this.apiService.send(
      "POST",
      "/organizations/" + organizationId + "/billing/payment-source/verify-bank-account",
      request,
      true,
      false,
    );
