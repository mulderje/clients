...
| `header`             | `TemplateRef<{ total, amountDue }>` | **Optional.** Custom header template to replace the default header                                                            |
...
export type Cart = {
  ...
  appliedBalance?: number; // Optional applied account balance (positive magnitude); rendered as a negative "Applied balance" row below the gross total
  amountDue?: number; // Optional amount charged after applied balance; when absent the renderer falls back to total
  total?: number; // Optional gross total; when absent the component computes it from the line items
};
...
Show a cart where Stripe applied part of the customer's account balance to the invoice. The gross
total, the applied balance (as a negative row), and the resulting amount due each render as their
own row, so the amount due is the figure the customer is actually charged:

<Canvas of={CartSummaryStories.WithAppliedBalance} />
