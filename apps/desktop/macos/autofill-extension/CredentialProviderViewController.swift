...
class CredentialProviderViewController: ASCredentialProviderViewController {
    ...
    override func prepareInterface(forPasskeyRegistration registrationRequest: ASCredentialRequest) {
        if let passkeyIdentity = registrationRequest.credentialIdentity as? ASPasskeyCredentialIdentity {
            if let passkeyRegistration = registrationRequest as? ASPasskeyCredentialRequest {
                ...
                client.preparePasskeyRegistration(request: req, callback: CallbackImpl(extensionContext))
                return
            }
        }

        // If we didn't get a passkey, return an error
        extensionContext.cancelRequest(withError: BitwardenError.Internal("Invalid registration request"))
    }

    override func prepareInterfaceToProvideCredential(for credentialRequest: ASCredentialRequest) {
        logger.log("prepare interface for credential request \(credentialRequest.description)")
    }

    // Deprecated
    override func prepareInterfaceToProvideCredential(for credentialIdentity: ASPasswordCredentialIdentity) {
        logger.log("prepareInterfaceToProvideCredential for credentialIdentity: \(credentialIdentity.user)")
    }
    ...
    override func prepareCredentialList(for serviceIdentifiers: [ASCredentialServiceIdentifier], requestParameters: ASPasskeyCredentialRequestParameters) {
        logger.log("prepareCredentialList(passkey) for serviceIdentifiers: \(serviceIdentifiers.count)")
